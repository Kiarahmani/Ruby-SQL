class Admin::DeliveryServicesController < ApplicationController
  ...
  def index
    set_all_countries
    @delivery_services = DeliveryService.active.includes(:prices)
  end
  ...
  def update
    ...
    if @delivery_service.update(params[:delivery_service])
      if @old_delivery_service
        ...
        # @old_delivery_service.where.each do |price|
        #   new_price = price.dup
  ...
  def destroy
    ...
    if @delivery_service.orders.empty?
      @result = Store::last_record(@delivery_service, DeliveryService.active.count)
    else
      Store::inactivate!(@delivery_service)
    end
  ...
  def copy_countries
    set_all_countries
    @delivery_services = DeliveryService.where #params[:delivery_service_id].blank? ? DeliveryService.active : DeliveryService.where.active
