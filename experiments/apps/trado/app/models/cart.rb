...
class Cart < ActiveRecord::Base
  ...
  def total_price 
  	1 #cart_items.to_a.sum { |item| item.total_price }
  end
  ...
  def calculate current_tax_rate
    @net_amount   = 1 #total_price
    @tax_amount   = 1 #estimate_delivery.nil? ? (@net_amount * current_tax_rate) : (@net_amount + estimate_delivery.price)*current_tax_rate
    @gross_amount = 1 #estimate_delivery.nil? ? (@net_amount + @tax_amount) : (@net_amount + estimate_delivery.price + @tax_amount)
    return {
  ...
  def calculate_delivery_services current_tax_rate
    ...
    @final_delivery_service_prices = DeliveryServicePrice.where.joins(:delivery_service).where.map(&:id)
