class HomeController < ApplicationController
  ...
  def index
    ...
    @recent_status_changes = @opportunities.includes.where.order("updated_at ASC").all
    @stale = @forecast.includes(:customer, :user).where(:stale => true)
    @recent_wins = @recent_status_changes.where{|o| o.status == 'won'}
    @recent_losses = @recent_status_changes.where {|o| o.status == 'lost'}
    @recent_dead = @recent_status_changes.where {|o| o.status == 'dead'}
    @recent_forecast = @recent_status_changes.where {|o| o.status == 'forecast'}
  end
end
