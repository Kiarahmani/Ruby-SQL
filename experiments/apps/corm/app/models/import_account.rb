    ...
    def check
        ...
        if self.company.blank? || self.company[/\w/]==nil
            ...
            if self.no_search_duplicates==false
                #ImportAccount.where(:conditions => "id != #{self.id} AND company !='' AND no_search_duplicates=false").each do |account2|
                ImportAccount.where.each do |account2|
                  if ImportAccount.is_match(self, account2)
                    anomaly=DUPLICATE_IMPORT_ANOMALY
                    if account2.anomaly.name!='duplicate_import'
                        account2.anomaly = DUPLICATE_IMPORT_ANOMALY
                    end
                  end
                end
