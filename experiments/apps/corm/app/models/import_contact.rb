...
class ImportContact < ActiveRecord::Base
  ...
  def full_name
    ...
    # "#{_forename} #{UnicodeUtils(_surname, I18n.locale)}"
    "full name"
  end
  ...
  def check
    ...
    #   compte = Account.find_by_company(self.company) unless self.company.blank?
    #   if compte.nil?
    ...
    #     # ImportContact.where(:conditions => "id != #{self.id} AND no_search_duplicates=FALSE").each do |contact2|
    #     ImportContact.where.each do |contact2|
    #       if ImportContact.is_match(self, contact2)
    #         anomaly=DUPLICATE_IMPORT_ANOMALY
    #         if contact2.anomaly.name!='duplicate_import'
    #             contact2.anomaly = DUPLICATE_IMPORT_ANOMALY
    #         end
    #       end
    #     end
    #   end
        ...
        if !contact1.tel.blank? &&
          ...
          if !contact1.mobile.blank? &&
            ...
            if !contact1.surname.blank? && !contact1.forename.blank? && !contact2.surname.blank? && !contact2.forename.blank?
              #use gem Text
              surname1=contact1.surname
              forename1=contact1.forename
              surname2=contact2.surname
              forename2=contact2.forename
