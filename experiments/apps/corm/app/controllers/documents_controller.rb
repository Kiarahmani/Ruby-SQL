...
class DocumentsController < ApplicationController
  ...
  def new
    ...
    @document.account = Account.find(params[:account_id])
    respond_to do |format|
  ...
  def create
    ...
    @document.created_by = current_user.id
		...
		if @document.save
			e = Event.new
      e.account_id = @document.account_id
			e.created_by = current_user.id
			e.notes = "Document n°#{@document.id} créé\r\nNom : #{@document.name}\r\nDescription : #{@document.notes}"
			e.user_id = current_user.id
			e.date_begin = Time.now
			e.date_end = Time.now
			e.save
  ...
  private
    ...
    def documents
      @account ? @account.documents : Document.all
    end
