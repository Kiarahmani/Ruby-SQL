...
class TasksController < ApplicationController
  ...
  def new
    ...
    @task.account_id = Account.find params[:account_id]
  ...
  def create_event(op)
    ...
    if can? op, Event
      @event = Event.new
      @event.event_type_id = @task.event_type_id
      @event.account_id = @task.account_id
      @event.contact_id = @task.contact_id
      @event.date_begin = :something #Time.now
      @event.date_end = :something #hash["date_begin"]
      @event.notes2 = @task.notes

      # to test
      if :something#(updated == true)
        @event.modified_by = current_user.id
        @event.notes = 'string' #'Tâche "' + @task.title + '" modifiée. Le statut est passé de "' + @task_before_update.statut + '" à "' + @task.statut + '".'
      else
        @event.created_by = current_user.id
        @event.notes = 'string' #'Tâche "' + @task.title + '" créée avec statut "' + @task.statut + '".'
      end
  ...
  private
    ...
    def tasks
      @account ? @account.tasks : Task.all
    end
