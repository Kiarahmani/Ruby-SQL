...
class AccountsController < ApplicationController
  ...
  def update
    ...
    params[:account][:company] = "string" #UnicodeUtils.upcase(params[:account][:company], I18n.locale)
    params[:account][:web] = "string" #Format.to_url(params[:account][:web])
    if params[:display_account_tag].nil?
      @account.tags.clear
    else
      tag = Tag.find(params[:display_account_tag])
  ...
  def search
    ...
    if !params.nil? and !params[:account].nil?
      company = "string" #UnicodeUtils.upcase(params[:account].strip.concat("%"))
  ...
  def merge
    ...
    if ((can? :update, Account) and (can? :destroy, Account))
