...
class QuotationsController < ApplicationController
  ...
  def get_companies
    ...
    # #                        .company#map { |quotation| quotation.company }
    # # else
    # #   companies = Quotation.select("DISTINCT quotations.company")
  ...
  def create_event(op)
    ...
    @event = Event.new#create(hash)
    @event.account_id = Account.find params[:quotation][:account_id]
    @event.contact_id = Contact.find params[:quotation][:contact_id]
    @event.date_begin = Time.now
    @event.date_end = params[:date_begin]
    @event.quotation_id = @quotation.id

    if op == :update
      @event.modified_by = current_user.id
      #@event.notes = "Devis n°" + @quotation.id.to_s + ' modifié. Le statut est passé de "' + @quotation_before_update.statut + '" à "' + params[:quotation][:statut] + '".'
    else
      @event.created_by = current_user.id
      @event.notes = "Devis n°" + @quotation.id.to_s + " créé."
    end
  ...
  private
    ...
    def quotations
      @account ? @account.quotations : Quotation.all
    end
