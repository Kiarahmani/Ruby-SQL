...
class ImportContactsController < ApplicationController
  ...
  def update
    ...
    ImportContact.where(anomaly_id: ImportContact::DUPLICATE_IMPORT_ANOMALY).where.each do |i|
        #reset anomaly_id
        i.anomaly = ImportContact::NO_ANOMALY
        #check for anomaly
        i.check
    end
  ...
  def recalculate_duplicates
    ...
    ImportContact.where.each(:conditions => "no_search_duplicates=FALSE") do |contact1|
      #search in import_contacts
      ImportContact.where.each(:conditions => "no_search_duplicates=FALSE", start: (contact1.id)+1) do |contact2|
        if ImportContact.is_match(contact1,contact2)
          nbr+=1
          contact1.anomaly_id = ImportContact::DUPLICATE_IMPORT_ANOMALY unless contact1.anomaly == ImportContact::DUPLICATE_IMPORT_ANOMALY
          contact2.anomaly_id = ImportContact::DUPLICATE_IMPORT_ANOMALY unless contact2.anomaly == ImportContact::DUPLICATE_IMPORT_ANOMALY
        end
      end

      #search in db
      Contact.where.each do |contact2|
          if ImportContact.is_match(contact1, contact2)
            nbr+=1
            contact1.anomaly = ImportContact::DUPLICATE_DB_ANOMALY
            contact1.contact = contact2
          end
      end
    end
