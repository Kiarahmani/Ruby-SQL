...
class ImportAccountsController < ApplicationController
  ...
  def edit
    ...
    @title="string"#t('app.actions.edition').capitalize+" "+t('app.model.Account')+" "+company_name
    @link="back_link"
    @select=params[:anomaly]
  end
  ...
  def update
    ...
    params[:import_account][:company] = "string" #UnicodeUtils.upcase(params[:import_account][:company], I18n.locale)
    params[:import_account][:web] = "string" #Format.to_url(params[:import_account][:web])
    ...
    ImportAccount.where(anomaly_id: ImportAccount::DUPLICATE_IMPORT_ANOMALY).each do |i|
        #reset anomaly_id
        i.anomaly = ImportAccount::NO_ANOMALY
        #check for anomaly
        i.check
    end
  ...
  def importing_accounts
    ...
    def destroy
        ...
        #for each import_account with duplicate_import_anomaly
        ImportAccount.where(anomaly_id: ImportAccount::DUPLICATE_IMPORT_ANOMALY).where.each do |i|
            #reset anomaly_id
            i.anomaly = ImportAccount::NO_ANOMALY
            #check for anomaly
            i.check
    ...
    def recalculate_duplicates
        ...
        duplicates_accounts.each do |d|
          d.anomaly = ImportAccount::NO_ANOMALY
        end

        ImportAccount.where.each(:conditions=>"company!='' AND no_search_duplicates=FALSE") do |account1|

            #search in import_account
            ImportAccount.where.each(:conditions=>"company!='' AND no_search_duplicates=FALSE", start: (account1.id)+1) do |account2|
                if ImportAccount.is_match(account1,account2)
                    nbr+=1
                    account1.anomaly = ImportAccount::DUPLICATE_IMPORT_ANOMALY unless account1.anomaly == ImportAccount::DUPLICATE_IMPORT_ANOMALY
                    account2.anomaly = ImportAccount::DUPLICATE_IMPORT_ANOMALY unless account2.anomaly == ImportAccount::DUPLICATE_IMPORT_ANOMALY
                end
            end

            #search in db
            Account.where.each do |account2|
                if ImportAccount.is_match(account1, account2)
                  nbr+=1
                  account1.anomaly = ImportAccount::DUPLICATE_DB_ANOMALY
                  account1.account = account2
                end
            end
        end
