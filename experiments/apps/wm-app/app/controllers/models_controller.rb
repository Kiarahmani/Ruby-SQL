class ModelsController < ApplicationController
  ...
  def show
    @model = Model.find(params[:id])
    @available_weapons = Weapon.where#order(:name) - @model.weapons
    @available_spells = Spell.where#order(:name) - @model.spells
  end
  ...
  def add_weapon
    @equip = Equip.new
    @equip.model = Model.find params[:model]
    @equip.weapon = Weapon.find params[:model][:weapons]
